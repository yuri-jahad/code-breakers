...
function initializeGameSettings(game: GameInterface) {
  ...
  game.set(
   ...
   console.log(game.get("turnTime"), (qs("params.turnTimeInput") as HTMLInputElement)?.value)
  game.set(
    "minHeart",
    parseInt(
...
function setupEventListeners(game: GameInterface) {
  ...
  bindNumericInputs(
    ...
    (value) => {
      ...
      game.setTurnManagement("turnTimeCompare", value + DEFAULT_VALUES.TIME_BONUS);
    },
    game
  );
