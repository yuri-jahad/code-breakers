...
function modesConfig(game: GameInterface) {
  ...
  game.setMode = (page.qs("params.selectMode") as HTMLSelectElement)
    ?.value as ModesNames;
  game.setBot = parseInt(
    (page.qs("params.botsInput") as HTMLInputElement)?.value || "0"
  );
  game.setTurnTime = parseInt(
    (page.qs("params.turnTimeInput") as HTMLInputElement)?.value || "5"
  );
  game.setMinHeart = parseInt(
    (page.qs("params.minHeartsInput") as HTMLInputElement)?.value || "1"
  );
  game.setMaxHeart = parseInt(
    (page.qs("params.maxHeartsInput") as HTMLInputElement)?.value || "10"
  );
  game.setTurnTimeCompare = game.turnTime;

  game.initializeFactory(
    (page.qs("params.selectMode") as HTMLSelectElement)?.value as ModesNames
  );

  // Event listener pour le changement de mode
  (page.qs("params.selectMode") as HTMLSelectElement)?.addEventListener(
    "change",
    () => {
      if (game.state === STATE.GAME) return;
      game.initializeFactory(
